...
f_stage2_cutoff  = 0.725e6;  % 阶段2/3分界 [Hz]（≈Mid段末，对应 fDATA_SoS 第二组末尾）
% -----------------------------------------------------------------------
...
for f_idx = 1:numel(fDATA)
    ...
    for iter_f_idx = 1:(niterSoSPerFreq(f_idx)+niterAttenPerFreq(f_idx))
        ...
        if enablePolarReg
            ...
            gradient_img   = gradient_img + lambda_polar * grad_scale_pol * (-div_r / div_r_scale);
        end
        % -----------------------------------------------------------------------
        ...
        fprintf('  alpha=%.2e | max|Δs|=%.2e | max|Δv|≈%.3f m/s\n', alpha, delta_s_max, delta_v_max);
        ...
        if saveGIF
            ...
            if mod(gif_iter_count - 1, gif_save_every) == 0
                ...
                title(axgif, sprintf('iter=%d  f=%.3fMHz  %s  pass=%s', ...
                    ...
                    ternary_str(updateAttenuation,'SoS+Atten','SoS')), 'FontSize', 10);
