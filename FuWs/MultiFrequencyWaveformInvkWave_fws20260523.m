...
%   11. [修改] kWave 步长改成 KCI 风格：
%              PolarPhase 使用从自身代价函数出发的 ΔA/Δφ residual-space 自洽线搜索；
%              线搜索分子/分母均在振幅/相位残差空间中计算，避免直接套用L2分母的量纲错配。
%              第7步 Armijo 回退暂不加入；删除 enableStepCap / alpha_cap / alpha_floor 这套 max|Δv| 人工夹紧。
...
%   32. [修改] A3最小修复：保留Atten dtheta梯度软加权，但L2原版链线搜索恢复Ali风格
%              未加权分母 -<g,d>/||Jd||²，避免把A3梯度预条件误写成加权L2目标函数。
%   33. [修改] L2原版链诊断打印加入 stageLabelCur，明确标注当前是Stage2 SoS还是Stage2 Atten更新。
% ===========================================================================================
clear; clc;
...
frac_excl_schedule          = [0.30, 0.25, 0.10];       % Stage1 / Stage2 / Stage3
...
misfitStage_SoSAtten_SoS = 'PolarPhase';          % Stage2：SoS+Atten里的SoS更新默认L2
...
misfitFreqRanges_SoSAtten_SoS = {[0.325e6, 0.525e6], 'PolarPhase';};
misfitFreqPoints_SoSAtten_SoS = {};

% Stage2 Atten子迭代
enableMisfitFreqSchedule_SoSAtten_Att = false;
misfitFreqRanges_SoSAtten_Att = {[1.100e6, 1.275e6], 'PolarPhase';};
...
% PolarPhase：当前流程（ΔA/Δφ residual-space 自洽线搜索 + TrustGate/CrossFreqTrust/EdgeGuard等门控）
% L2        ：VE+L2原版流程（原CG + 原线搜索 + 原慢度更新，无额外保护项改写）
% VE通道降权（beta_ve_cur）两条链均生效
...
% -----------------------------------------------------------------------
% ------------------ fws: 优化器设置 ------------------
% 1) L2：仍使用 Ali 原版线性化线搜索 alpha_ls = -<g,d>/||Jd||²
% 2) PolarPhase：使用 ΔA / Δφ residual-space 自洽线搜索：
%       先把 Born扰动 dREC_SIM 分解为振幅扰动 δA 和相位扰动 δφ，
%       再在 PolarPhase 代价函数的一阶线性化下计算 alpha_ls。
%       本版暂不加入 Armijo 回退（即不做额外试探正演）。
% 3) [修改] 删除 max|Δv| 人工夹紧：
...
for f_idx = 1:numel(fDATA)
    for iter_f_idx = 1:(niterSoSPerFreq(f_idx)+niterAttenPerFreq(f_idx))
        ...
        % 注意：A3在本版本中作为Atten梯度预条件/软加权，不改写L2原版线搜索分母。
        %       REC_WEIGHT_CUR 仅供PolarPhase自洽线搜索/诊断使用；L2链保持Ali风格未加权分母。
        ...
        if useL2OriginalChain
            % ================================================================
            % [修改] fws: A3最小修复——L2原版链恢复Ali风格未加权线搜索
            % ================================================================
            % A3的dtheta权重只作为Atten梯度软加权/预条件：
            %   - ADJ_SRC中仍按 VE置信度 + Atten角度权重构建加权伴随源；
            %   - 但L2线搜索分母保持原版 ||Jd||²，不再乘 REC_WEIGHT_CUR。
            %
            % 这样避免把"Atten梯度软加权"误变成"加权L2目标函数"，
            % 同时保留 Stage2 SoS/Atten 统一L2 的自洽线搜索尺度。
            ...
            if updateAttenuation && enableAttenAngleWeight
                fprintf(['  [L2原版VE链 | %s | β=%.2f + A3] Φ=%.3e | ΔΦ=% .3e | τ=%.3eus | R=%.3f' ...
                ...
                fprintf(['  [L2原版VE链 | %s | β=%.2f] Φ=%.3e | ΔΦ=% .3e | τ=%.3eus | R=%.3f' ...
                         ' | alpha=%.2e | max|Δv|≈%.3f m/s\n'], ...
                    stageLabelCur, beta_ve_cur, phi_k, delta_phi_k, tau_k*1e6, ring_k, alpha, delta_v_max);
            end
        else

        % 统一线搜索（L2形式，作为非PolarPhase或分母退化时的fallback）
        den_ls   = real(dREC_SIM(:)'*dREC_SIM(:));
        num_ls   = -real(gradient_img(:)'*search_dir(:));
        alpha_ls = num_ls / (den_ls + eps);
        alpha_ls_fallback = alpha_ls;

        % ================================================================
        % fws: PolarPhase residual-space 自洽线搜索（Claude推导版，第7步Armijo暂不加入）
        % ----------------------------------------------------------------
        % 思路：
        %   参考L2线搜索的 residual-space 二次近似，但不再使用 ||δd||² 作为分母。
        %   对当前计算数据 p_sim 沿搜索方向的 Born 扰动 δp=dREC_SIM 做一阶分解：
        %
        %       δA   = Re(conj(e_hat) * δp)
        %       δφ   = Im(conj(e_hat) * δp) / |p_sim|
        %       e_hat = p_sim / |p_sim|
        %
        %   SoS阶段：
        %       r_amp = |p_sim| - |p_obs|，dr_amp = δA
        %   Atten阶段：
        %       r_amp = log|p_sim| - log|p_obs|，dr_amp = δA / |p_sim|
        %
        %   当前代码的残差定义为 sim-obs，因此线性化残差为 r(α)=r0+α·dr，
        %   最优步长为：
        %
        %       alpha_PP = - <r_polar, dr_polar> / ||dr_polar||²
        %
        %   若采用 Claude 文档中的 obs-sim 残差记号，则等价写成正号。
        %
        % 权重：
        %   使用 (1-alpha_hv_ls_cur) 与 alpha_hv_ls_cur 的一次权重，
        %   对应 J_PP = 1/2 Σ[(1-αhv)ΔA² + αhvΔφ²] 的线性化步长；
        %   不再把权重平方后放入分子/分母。
        %
        % 通道权重：
        %   REC_WEIGHT_CUR 来自 VE置信度 beta_ve_cur 以及 Atten角度软加权A3；
        %   这里作为线搜索中的通道权重使用，与ADJ_SRC中的加权保持同一量级。
        % ================================================================
        if strcmpi(misfitType_cur, 'PolarPhase')
            eps_safe_pp = 1e-12;
            num_pp = 0;
            ...
            for elmt_idx = 1:numel(tx_include_cur)
                ...
                dpsi     = dREC_SIM(elmt_idx, mask).';          % δp = Jp
                phi_d    = phi_dir_store(elmt_idx, mask).';     % e_hat
                A_sim    = double(amp_sim_store(elmt_idx, mask).');
                r_amp    = double(amp_res_store(elmt_idx, mask).');
                r_phi    = double(dphi_store(elmt_idx, mask).');

                A_safe = max(A_sim, eps_safe_pp);

                % δp 分解到振幅方向和相位方向
                eH_dpsi   = conj(phi_d) .* dpsi;
                delta_A   = real(eH_dpsi);              % δA
                delta_phi = imag(eH_dpsi) ./ A_safe;    % δφ [rad]

                % Atten阶段使用 log-amplitude 残差，因此幅度导数也要除以 |p_sim|
                if updateAttenuation
                    delta_amp = delta_A ./ A_safe;      % δlogA
                else
                    delta_amp = delta_A;                % δA
                end

                % fws: 线搜索通道权重（VE beta + Atten角度软权重）
                w_ls = double(REC_WEIGHT_CUR(elmt_idx, mask).');
                if isempty(w_ls) || all(w_ls == 0)
                    w_ls = ones(size(delta_amp));
                end

                w_amp = (1 - alpha_hv_ls_cur);
                w_phi = alpha_hv_ls_cur;

                num_pp = num_pp + sum(w_ls .* ( ...
                    w_amp .* r_amp .* delta_amp + ...
                    w_phi .* r_phi .* delta_phi ), 'omitnan');

                den_pp = den_pp + sum(w_ls .* ( ...
                    w_amp .* delta_amp.^2 + ...
                    w_phi .* delta_phi.^2 ), 'omitnan');
            end

            if ~isfinite(den_pp) || abs(den_pp) < eps_safe_pp
                fprintf('  [PolarPhase-LS] den_pp过小或非法，回退到L2形式alpha_ls_fallback=%.2e。\n', ...
                    alpha_ls_fallback);
                alpha_ls = alpha_ls_fallback;
            else
                alpha_ls = -real(num_pp) / (real(den_pp) + eps_safe_pp);
            end
        end

        % 步长处理（删除 max|Δv| 人工夹紧）
        % [修改] PolarPhase 直接采用 residual-space 自洽线搜索得到的 alpha_ls；
        %        不再使用 auto_dv_ratio / target_dv_per_iter / alpha_cap 截断。
        %        max|Δv| 后面仅作为日志诊断量显示。
        alpha = alpha_ls;

        % 非法或非正步长：不再使用 alpha_floor 强行微步更新
        if ~isfinite(alpha) || (alpha <= 0)
            fprintf('  [PolarPhase-LS] alpha_ls=%.2e 非正或非法，本轮跳过模型更新（alpha=0）。\n', alpha_ls);
            alpha = 0;
        end
        ...
        if updateAttenuation && enableAttenAngleWeight
            fprintf(['  [%s] Φ=%.3e | ΔΦ=% .3e | τ=%.3eus | R=%.3f | γ=% .3f' ...
                     ...
                     ' | alpha_ls=%.2e' ...
                     ' | alpha=%.2e(gate=%.2f,xf=%.2f) | report max|Δv|≈%.3f m/s' ...
