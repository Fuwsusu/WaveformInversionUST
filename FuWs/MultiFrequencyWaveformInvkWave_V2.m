clear
clc
scriptTimer = tic;   % 整个脚本总时长（含预处理、主反演、保存）
...
% % Extract the Desired Frequency for Waveform Inversion  
% fDATA_SoS = (0.3:0.02:1.25)*(1e6); % Frequencies for SoS-only Iterations [Hz]
% fDATA_SoSAtten = (0.325:0.02:1.275)*(1e6); % Frequencies for SoS/Attenuation Iterations [Hz]
% fDATA = [fDATA_SoS, fDATA_SoSAtten]; % All Frequencies [Hz]
...
% fDATA_SoS = [ (0.30:0.05:0.60), (0.63:0.04:0.90), (0.93:0.03:1.25) ] * 1e6;
...
% modify fws20251125_half2
% 按阵元总数的 1/4 剔除近端通道（两边各 1/8）
frac_excl = 1/4;  % 剔除总通道的 1/4
numElemLeftRightExcl = round(numElements * frac_excl / 2);  % 每侧
elemLeftRightExcl = -numElemLeftRightExcl:numElemLeftRightExcl;
elemInclude = true(numElements, numElements);
for tx_element = 1:numElements
    elemLeftRightExclCurrent = elemLeftRightExcl + tx_element;
    elemLeftRightExclCurrent(elemLeftRightExclCurrent<1) = ...
        numElements + elemLeftRightExclCurrent(elemLeftRightExclCurrent<1);
    elemLeftRightExclCurrent(elemLeftRightExclCurrent>numElements) = ...
        elemLeftRightExclCurrent(elemLeftRightExclCurrent>numElements) - numElements;
    elemInclude(tx_element, elemLeftRightExclCurrent) = false;
end
...
%% ---------- 用户控制：迭代执行选项 ----------
runAllIterations = false;    % true=跑满 Niter；false=按 requestedIterations 提前停止
requestedIterations = 60;    % 想跑多少次“全局迭代”（跨频率累计）
...
for f_idx = 1:numel(fDATA)
    ...
    for iter_f_idx = 1:(niterSoSPerFreq(f_idx)+niterAttenPerFreq(f_idx))
        ...
        % ---------- 迭代次数控制：达到 requestedIterations 就停止 ----------
        if ~runAllIterations && (iter > requestedIterations)
            stopNow = true;
            iter = requestedIterations; % 保证后续显示/标题使用最后一次“已完成”的迭代编号
            disp(['Stopped early after ', num2str(requestedIterations), ' iterations.']);
            break;   % 跳出当前 iter_f_idx 循环
        end
        % -------------------------------------------------------------------
        ...
        % %fws- Added: Safe line search to prevent division by zero / NaN
        % perc_step_size = 0.5;  % smaller step = more stable
        % num_alpha = -(gradient_img(:)' * search_dir(:));
        % den_alpha = (dREC_SIM(:)' * dREC_SIM(:));
        % if abs(den_alpha) < 1e-10 || ~isfinite(den_alpha)
        %     den_alpha = 1e-10;
        % end
        % alpha = num_alpha / den_alpha;
        % % limit large / invalid alpha
        % if ~isfinite(alpha) || abs(alpha) > 5
        %     alpha = 0;
        % end
...
% 在SOS图上给出一条水平切线，并对比该切线上的重建/真值声速曲线
figure(6); clf;

cut_y_m = 0.01;          % 水平切线 y 坐标 [m]
lineColor = [1 1 0];     % 切线颜色（黄色）

[~, iy_recon] = min(abs(yi - cut_y_m));
[~, iy_true]  = min(abs(yi_orig - cut_y_m));

vel_prof_est  = VEL_ESTIM(iy_recon, :);
vel_prof_true = C(iy_true, :);

subplot(1,2,1);
imagesc(xi, yi, VEL_ESTIM, crange);
axis image; colorbar; colormap(cmap_rb);
hold on;
plot([xi(1), xi(end)], [yi(iy_recon), yi(iy_recon)], '-', 'Color', lineColor, 'LineWidth', 1.5);
hold off;
xlabel('Lateral [m]'); ylabel('Axial [m]');
title(sprintf('Estimated SOS with horizontal line (y = %.1f mm)', yi(iy_recon)*1e3));

subplot(1,2,2);
plot(xi_orig, vel_prof_true, 'k-', 'LineWidth', 2.0, 'DisplayName', 'True'); hold on;
plot(xi,      vel_prof_est,  'r-', 'LineWidth', 1.8, 'DisplayName', 'Estimated');
grid on;
xlabel('Lateral position [m]');
ylabel('Sound Speed [m/s]');
title(sprintf('Horizontal SOS profile at y = %.1f mm', yi(iy_recon)*1e3));
