...
requestedIterations = 180; % Only effective when runAllIterations=false
...
% Priority: Ali paper-style 512-element reconstruction result (as reference truth)
hasGroundTruth = false;
groundTruthSource = 'None';
useAli512AsTrue = true;
hasGroundTruthImage = false;
groundTruthImage = [];
aliTrueGifPath = 'D:\\Document_ING_fws\\WaveformInversionUST\\Results-paper\\BenignCyst.gif';

% Remove sparse suffix (e.g., BenignCyst_sparse256 -> BenignCyst)
baseFilename = regexprep(filename, '_sparse\d+$', '');

% Candidate paths for Ali 512-element reference result
aliTrueFileCandidates = {
    fullfile('SampleData', [baseFilename, '.mat']), ...
    fullfile('FuWs', 'SampleData', [baseFilename, '.mat']), ...
    fullfile('Results', [baseFilename, '_WaveformInversionResults_512.mat']), ...
    fullfile('Results', [baseFilename, '_WaveformInversionResults.mat']) ...
};

if useAli512AsTrue
    for k_true = 1:numel(aliTrueFileCandidates)
        truePath = aliTrueFileCandidates{k_true};
        if exist(truePath, 'file') ~= 2
            continue;
        end

        varsTrue = who('-file', truePath);

        % Case 1: Ali dataset file contains direct ground truth fields
        if all(ismember({'C', 'xi_orig', 'yi_orig'}, varsTrue))
            load(truePath, 'C', 'xi_orig', 'yi_orig');
            hasGroundTruth = true;
            groundTruthSource = sprintf('Ali-512 dataset GT: %s', truePath);
            break;
        end

        % Case 2: reference reconstruction result file (use final map as truth)
        if all(ismember({'VEL_ESTIM_ITER', 'xi', 'yi'}, varsTrue))
            Strue = load(truePath, 'VEL_ESTIM_ITER', 'xi', 'yi');
            C = Strue.VEL_ESTIM_ITER(:,:,end);
            xi_orig = Strue.xi;
            yi_orig = Strue.yi;
            hasGroundTruth = true;
            groundTruthSource = sprintf('Ali-512 reconstruction result: %s', truePath);
            break;
        end

        % Case 3: a single final reconstruction map variable
        singleMapCandidates = {'VEL_ESTIM', 'VEL_FINAL', 'c_map', 'C_true'};
        mapFound = false;
        for vIdx = 1:numel(singleMapCandidates)
            vName = singleMapCandidates{vIdx};
            if ismember(vName, varsTrue)
                Strue = load(truePath, vName);
                C = Strue.(vName);
                if ~(ismember('xi', varsTrue) && ismember('yi', varsTrue))
                    continue;
                end
                tmpXY = load(truePath, 'xi', 'yi');
                xi_orig = tmpXY.xi;
                yi_orig = tmpXY.yi;
                hasGroundTruth = true;
                groundTruthSource = sprintf('Ali-512 reference map (%s): %s', vName, truePath);
                mapFound = true;
                break;
            end
        end
...
% Fallback: try GT fields from current data file
if ~hasGroundTruth && exist(dataPath, 'file') == 2
    varsInFile = who('-file', dataPath);
    if all(ismember({'C', 'xi_orig', 'yi_orig'}, varsInFile))
        load(dataPath, 'C', 'xi_orig', 'yi_orig');
        hasGroundTruth = true;
        groundTruthSource = sprintf('Current dataset GT: %s', dataPath);
    end
end

% Fallback-2: if only Ali 512 GIF is available, use the last frame as image-domain truth
if ~hasGroundTruth && exist(aliTrueGifPath, 'file') == 2
    try
        gifInfo = imfinfo(aliTrueGifPath);
        gifLastFrame = numel(gifInfo);
        [A_gif, map_gif] = imread(aliTrueGifPath, 'gif', 'Frames', gifLastFrame);
        if ~isempty(map_gif)
            A_gif = ind2gray(A_gif, map_gif);
        end
        if ndims(A_gif) == 3
            A_gif = rgb2gray(A_gif);
        end
        groundTruthImage = im2double(A_gif);
        hasGroundTruthImage = true;
        groundTruthSource = sprintf('Ali-512 GIF (last frame=%d): %s', gifLastFrame, aliTrueGifPath);
    catch ME
        hasGroundTruthImage = false;
        warning('GT:GifLoadFailed', 'Failed to load GIF GT: %s', ME.message);
    end
end

if hasGroundTruth
    fprintf('[GT] using reference: %s\n', groundTruthSource);
elseif hasGroundTruthImage
    fprintf('[GT] using image reference: %s\n', groundTruthSource);
else
    fprintf('[GT] reference truth not found, metrics will use no-GT mode.\n');
end
...
if hasGroundTruth
    ...
    fprintf('\n====== Quantitative Metrics (with GT / Ali-512 reference) ======\n');
    fprintf('PSNR : %.4f dB\n', PSNR_dB);
    fprintf('RMSE : %.6f m/s\n', RMSE_val);
    fprintf('SSIM : %.6f\n', SSIM_val);
    fprintf('RD   : %.4f %%\n', RD_percent);
    fprintf('===============================================================\n\n');
elseif hasGroundTruthImage
    % Image-domain metrics against Ali GIF last frame (no physical units)
    estNorm = mat2gray(VEL_ESTIM, crange);
    gtImg = groundTruthImage;
    if ~isequal(size(gtImg), size(estNorm))
        gtImg = imresize(gtImg, size(estNorm), 'bilinear');
    end

    diffImg = estNorm - gtImg;
    mse_img = mean(diffImg(:).^2);
    rmse_img = sqrt(mse_img);
    if mse_img > 0
    ...
    var_t = var(gtImg(:),1);
    var_e = var(estNorm(:),1);
    cov_te = mean((gtImg(:)-mu_t).*(estNorm(:)-mu_e));
    C1_ssim = (0.01)^2;
    C2_ssim = (0.03)^2;
    ssim_img = ((2*mu_t*mu_e + C1_ssim) * (2*cov_te + C2_ssim)) / ...
               ((mu_t^2 + mu_e^2 + C1_ssim) * (var_t + var_e + C2_ssim));

    fprintf('\n==== Quantitative Metrics (Ali-512 GIF image-domain reference) ===\n');
    fprintf('PSNR(img) : %.4f dB\n', psnr_img);
    fprintf('RMSE(img) : %.6f\n', rmse_img);
    fprintf('SSIM(img) : %.6f\n', ssim_img);
    fprintf('===============================================================\n\n');
else
